...
class RendezVous {
    ...
    public function listAll($idUser){
        $requete = "SELECT *, DATE_FORMAT(date, '%d/%m/%Y') as dateEu, DATE_FORMAT(start_hour, '%Hh%i') as hourEu, NOW() as thisDay FROM RendezVous WHERE Rendezvous.user_id = ? ORDER BY date DESC LIMIT 10 ";
